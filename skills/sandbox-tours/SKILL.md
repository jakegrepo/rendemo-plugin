---
name: sandbox-tours
description: This skill should be used when a product tour will run on a SANDBOX — a site Rendemo crawled from a URL into a replica we host — rather than on the user's own repo. Use it when the user says "make a tour of <some website>", gives a URL and no codebase, mentions `rendemo_crawl_site`, a crawl, a sandbox or a replica, or is authoring against an existing sandbox demo — and before `rendemo_add_tour_step`'s `sandbox` form, `rendemo_list_sandbox_pages`, `rendemo_find_in_sandbox` or `rendemo_check_sandbox_tour`. A sandbox tour writes no `data-rendemo` markers into source, has no lockfile to commit, and publishes in an order that fails with 409 sandbox_not_published if taken in the obvious sequence, so it is not the `author-a-tour` procedure with the repo swapped out. Covers surveying what the crawl actually captured, choosing steps and target elements that survive a rebuild, writing the step card, and verifying before publish.
version: 1.0.0
---

<!-- GENERATED by `npm run skills:generate` from lib/copilot/skills/sandbox-tours/SKILL.md.
     Edit that file, not this one. `npm run skills:check` fails the build if they have drifted. -->

# Sandbox tours

A sandbox tour and a codebase tour look identical to the visitor and are authored almost nothing
alike. On a codebase tour you write a `data-rendemo` marker into the customer's source and the
...
`tour-craft` still governs how a step *behaves*: emphasis modes, the magnified lens, the camera on
an `explain` step, which target relationships are inert. Read it too. This skill is the part that is
only true when there is no repo.
...
- **The sandbox comes back unpublished, and authoring never needs it published.** Every read tool
  and `rendemo_add_tour_step` work on a draft sandbox. Publishing it puts a replica of someone
...
when refreshing a site you already have tours on. A fresh crawl of the same URL is a different
sandbox, and tours pointed at the old one keep pointing at the old one.
...
**This list is the whole world.** A page that is not in it does not exist for this tour — not
...
read the list as a statement of what the tour *can* be about, before you have a story in mind. If
...
This is the part no tool checks and the part that decides whether the tour is any good. A tour is
not a table of contents for the crawl. Some discipline that survives contact with a real replica:

- **Follow one visitor's path, and start where they start.** The `entry` page is where the tour
...
- **Every step must answer "and then what?"** The failure mode of an auto-authored tour is a step
...
- **Chrome is not content.** Cookie banners, consent dialogs, newsletter modals, social icons,
  ...
  subject of a step.
- **Three to seven steps is a tour. Fifteen is a sitemap with cards on it.** Spend the count on the
...
- **Point at the control, not its wrapper.** A tour's geometry is live: the highlight is exactly the
...
A tour honours a fraction of the player's card surface. These render:
...
`rendemo_add_tour_step` takes `title`, `blurb`, `emphasis` and `lens` inline — dress the step as you
...
- **`stepType` renders nowhere, and is not inert.** On a sandbox tour it is what gates the camera:
  only an `explain` step is eligible for `rendemo_direct_step`'s `shot` / `settleShot`. A step left
  `action` stays at 1x no matter how it is framed. See `tour-craft` for the camera itself, which is
  worth two or three steps in a tour and no more.

Emphasis follows the same scarcity rule as everywhere else: `ring` for most steps, `brackets` for a
region, and `spotlight` or `frost` for the one or two steps the tour exists to reach. A dim on every
step is shouting.

## Verify, then publish in the right order

**1. `rendemo_check_sandbox_tour`.** Re-scores every sandbox-anchored step against the sandbox's
...
before publishing. It refuses on a tour with no sandbox-anchored steps, which is itself a useful
signal that you authored `route` steps by mistake.

**2. `rendemo_get_tour_preview`** when a reviewer is ready to look. The link is a bearer token —
...
**3. Publish the SANDBOX, then the tour.** This is the ordering that catches people, because it
looks like it contradicts the rule above it. Authoring never needs a published sandbox. *Publishing
the tour does*: the tour has nothing to run on until the replica it points at is itself live, so
`rendemo_publish_demo` on a sandbox tour returns **409 `sandbox_not_published`** if the sandbox is
still a draft or has been taken down. Publishing a sandbox is world-visible and confirm-gated — ask
first, every time.

Four more refusals, all deliberate, all 422 unless noted:

- **`sandbox_tour_mixed_targets`** — some steps are sandbox-shaped and some are plain `route` steps.
  `rendemo_add_tour_step` only checks that `route` and `sandbox` are exclusive *within one call*, so
  a mixed tour is easy to author and impossible to publish. The response names every step and its
  shape.
- **`sandbox_tour_mixed_sandboxes`** — the steps name more than one sandbox. Refused rather than
...
- **`sandbox_tour_unresolved`** — at least one step did not stamp. The issues list gives each
  marker, its page and its code; this is exactly what step 1 exists to catch earlier.
- **`sandbox_strip_refused`** (409) — a page carrying a stale marker from a previous tour version
...
After any step change the tour must be published again — until then the live tour still describes
the old shape.

## Tools

`rendemo_crawl_site` → `rendemo_get_crawl_status` → `rendemo_list_sandbox_pages` →
`rendemo_find_in_sandbox` / `rendemo_read_sandbox_page` → `rendemo_create_tour` →
`rendemo_add_tour_step` (`sandbox: { demoId, page, node }`) → `rendemo_update_step` /
`rendemo_set_step_presentation` / `rendemo_direct_step` → `rendemo_check_sandbox_tour` →
`rendemo_get_tour_preview` → `rendemo_publish_sandbox` → `rendemo_publish_demo`.

`sandbox.demoId` is the **sandbox's** id, never the tour project's own — the two are different rows
